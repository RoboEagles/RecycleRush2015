...
public class liftingMechanism extends PIDSubsystem {
    ...
    public static double targetHeight = 0.0; // TARGET HEIGHT IN INCHES
    private double targetRate = 6.0,  // TARGET RATE IN INCHES/SEC
    		toteRaiseHeight = 13.0,
    		containerRaiseHeight = 26.0;
    ...
    public void raiseNext() {
    	double raiseHeight = targetHeight > 0.0 ? toteRaiseHeight : containerRaiseHeight;
    	if (raiseHeight < 78) {
    		setHeight(targetHeight + raiseHeight);
    	}
    }
    
    public void setDrive() {
    	double y = oi.getliftStick().getY();
    	y = -1*(y/Math.abs(y))*Math.pow(y,2);
    	//yOut = yOutOld + lpfK*(y - yOutOld);
    	//yOut = Math.abs(yOut) > 0.5 ? 0.5 : yOut;
    	liftMotor.set(y);
    	//yOutOld = yOut;
